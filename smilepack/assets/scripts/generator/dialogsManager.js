...
dialogsManager.init = function(background, dialogs){
    ...
    for(var name in dialogs) this.register(name, dialogs[name]);
};


dialogsManager.register = function(name, dialog){
    if(this.dialogs[name]) throw new Error('Conflict dialog ' + name);
    this.dialogs[name] = {
        dialog: dialog,
        submitCallback: null
    };
...
dialogsManager.open = function(name, options, onsubmit){
    if(!this.dialogs[name]) throw new Error('Unknown dialog ' + name);
...
dialogsManager.close = function(name){
    if(!this.dialogs[name]) throw new Error('Unknown dialog ' + name);
    if(!this.opened.indexOf(name) < 0) return null;
...
dialogsManager._openEvent = function(name, opened){
    ...
    if(this.background){
        if(this.opened.length > 0) this.background.classList.remove('hidden');
        else this.background.classList.add('hidden');
    }
};
