...
var dragdrop = {
    ...
    add: function(container, options){
        ...
        var mousedown = function(event){return this._eventDown(event, options)}.bind(this);
    ...
    _stop: function(dropAction, ontransitionend){
        if(this._currentElement) this._currentElement.removeEventListener('click', this._eventPrevent);
        ...
        if(dropAction && dropAction.name == 'animate' && dropAction.targetElement) {
            ...
            if(ontransitionend) ontransitionend({element: dropAction ? dropAction.targetElement : null});
        }
        ...
        if(document.body.classList.contains('drag-enabled')) document.body.classList.remove('drag-enabled');
        if(this._callbacks){
    ...
    _animate: function(targetElement, ontransitionend){
        ...
        var endAnimation = function(event){
            if(event && event.propertyName != 'top' && event.propertyName != 'left') return;
